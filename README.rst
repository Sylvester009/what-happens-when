...
- USB Protocol: The USB packets generated by the USB SIE follow the USB protocol,
  which defines how devices communicate over USB connections. The protocol includes
  specifications for data transfer, device enumeration, power management,
  and error handling.
...
- The virtual keyboard can now raise a software interrupt for sending a
  'key pressed' message back to the OS. And on touchscreen, the touchscreen
  controller detects the touch input and communicates the coordinates of the
  touch event to the operating system. The operating system then interprets 
  the touch event as a keypress and handles it similarly to a physical 
  keypress from a traditional keyboard.
